...
class NeuralNetwork {
    ...
    /**
     * Computes the output for the given input using the feedforward algorithm for
     * each layer in the neural network. output = sigmoid (weights*input + bias)
     * 
     * @param imageArray color representation of the 28x28 pixel image in a row-wise
     *                   format
    ...
    public double[] feedforward(Color[] imageArray) {
        ...
        outputMatrices[0] = (weightMatrices[0].mult(inputMatrix)).plus(biasMatrices[0]); // Set the product of
                                                                                         // weightMatrices[0] and
                                                                                         // inputMatrix[0] plus
                                                                                         // biasMatrices[0] of hidden
                                                                                         // layer to outputMatrices[0]

        for (int i = 0; i < outputMatrices[0].numRows(); i++) { // For loop that loops over outputMatrices[0]
            double value = sigmoid(outputMatrices[0].get(i, 0)); // Sets the sigmoid of the value of hidden layer output
                                                                 // matrix to the double value.
            outputMatrices[0].set(i, 0, value); // New value of sigmoid function set back into hidden layer output
                                                // matrix
        }

        outputMatrices[1] = (weightMatrices[1].mult(outputMatrices[0])).plus(biasMatrices[1]); // Set the product of
                                                                                               // weightMatrices[1] and
                                                                                               // inputMatrix[1] plus
                                                                                               // biasMatrices[1] of
                                                                                               // hidden layer to
                                                                                               // outputMatrices[1]
        ...
        for (int i = 0; i < outputMatrices[1].numRows(); i++) { // For loop that loops over outputMatrices[1]
            double value = sigmoid(outputMatrices[1].get(i, 0)); // Sets the sigmoid of the value of output layer matrix
                                                                 // to the double value.
        ...
        for (int i = 0; i < OUTPUTS; i++) { // For loop that loops over OUTPUTS
            outputArray[i] = outputMatrices[1].get(i, 0); // Get the output layer matrix values and set them at each
                                                          // outputArray index
        }
    ...
    public void train(Color[] imageArray, int[] answers) throws IOException {
        ...
        /*
         * calculate and store errors in the output layer outputError = output - answer
         */
        errorMatrices[1] = outputMatrices[1].minus(answersMatrix);

        /*
         * calculate and store errors in the hidden layer based on the error in the
         * output layer hiddenError = transpose(weights) * outputError
         */
        ...
        /*
         * define weight deltas according to the rule: deltaWeight = error *
         * transpose(input)
         */
    ...
    /*
     * Sigmoid Function Normalizes all real numbers to within 0 and 1
     */
     ...
     * (p>>16) & 0xff - red (p >> 8) & 0xff - green (p) & 0xff - blue ( (0.3 * R) +
     * (0.59 * G) + (0.11 * B) )
     */
    ...
    /*
     * Scales a value in one range to another 0 - 255 0 - 1 x1 0 x2 255 y1 0 y2 1
     */
